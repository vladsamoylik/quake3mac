...
================
*/
unsigned int S_HashSFXName(const char *name) {
	unsigned int hash;
	char	letter;
	int		i;
...
static sfxHandle_t S_Base_RegisterSound( const char *name, qboolean compressed ) {
	...
	if ( sfx->soundData ) {
		...
		// Update cache timestamp for existing sound
		extern void S_UpdateSoundCache(sfx_t *sfx);
		S_UpdateSoundCache(sfx);
		return sfx - s_knownSfx;
	}

	// Check sound cache first
	extern sfx_t *S_FindCachedSound(const char *name);
	sfx_t *cachedSfx = S_FindCachedSound(name);
	if (cachedSfx && cachedSfx->soundData && !cachedSfx->defaultSound) {
		// Use cached sound data
		sfx->soundData = cachedSfx->soundData;
		sfx->soundLength = cachedSfx->soundLength;
		sfx->soundChannels = cachedSfx->soundChannels;
		sfx->soundCompressed = cachedSfx->soundCompressed;
		sfx->soundCompressionMethod = cachedSfx->soundCompressionMethod;
		sfx->inMemory = qtrue;
	...
	// Queue sound for asynchronous loading instead of immediate loading
	extern void S_QueueSoundLoad(sfx_t *sfx, const char *filename);
	S_QueueSoundLoad(sfx, name);

	// For now, load immediately if needed (fallback)
	// TODO: Make this truly asynchronous by deferring sound playback until loaded
	if (!sfx->soundData) {
		S_memoryLoad(sfx);

		// Add to cache after successful load
		if (sfx->soundData && !sfx->defaultSound) {
			extern void S_AddToSoundCache(sfx_t *sfx);
			S_AddToSoundCache(sfx);
		}
	}
...
qboolean S_Base_Init( soundInterface_t *si ) {
	...
	Cvar_SetDescription( s_khz, "Specifies the sound sampling rate, (8, 11, 22, 44, 48) in kHz. Default value is 48 (professional quality)." );
	...
	s_mixahead = Cvar_Get( "s_mixAhead", "0.1", CVAR_ARCHIVE_ND );
	Cvar_CheckRange( s_mixahead, "0.001", "0.5", CV_FLOAT );
	Cvar_SetDescription( s_mixahead, "Amount of time to pre-mix sound data to avoid potential skips/stuttering in case of unstable framerate. Higher values add more CPU usage. Default 0.1s for low latency." );
