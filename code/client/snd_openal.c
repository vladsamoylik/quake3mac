...
Copyright (C) 2024 Quake3mac contributors

OpenAL sound backend for Quake3mac
Based on ioquake3 architecture with full feature parity
...
static qboolean alInitialized = qfalse;

//===========================================================================
// Sound Effect Buffer Management
//===========================================================================
...
typedef struct alSfx_s {
	...
	ALuint buffer;
	snd_info_t info;            // Sound info (rate, samples, etc.)
	
	qboolean isDefault;         // Using default sound
	qboolean isDefaultChecked;  // Already checked for default
	qboolean inMemory;          // Buffer loaded in memory
	qboolean isLocked;          // Can't be unloaded
	int lastUsedTime;
	
	// Loop synchronization (from ioq3)
	int loopCnt;                // Number of loops using this sfx
	int loopActiveCnt;          // Number of actively playing loops
	int masterLoopSrc;          // Master source for loop sync
} alSfx_t;
...
typedef struct src_s {
	...
	qboolean isPlaying;         // Is this source currently playing?
	qboolean isLocked;          // This is locked (un-allocatable)
	qboolean isLooping;         // Is this a looping effect?
	qboolean isTracking;        // Is this object tracking its owner?
	qboolean isStream;          // Is this source a stream?
	
	float curGain;              // Gain if source is within maxdistance
	float scaleGain;            // Last gain value for this source
	
	float lastTimePos;          // On stopped loops, last position in buffer
...
// Free list for O(1) source allocation
static int srcFreeList[MAX_SRC];
static int srcFreeCount = 0;
...
// Raw samples streaming (cinematics)
...
// Underwater effect
static qboolean listenerUnderwater = qfalse;
#define UNDERWATER_GAIN_FACTOR 0.5f

// Third person detection threshold
#define THIRD_PERSON_THRESHOLD_SQ (48.0f * 48.0f)

// Forward declarations
static void S_AL_RawSamplesEx(int stream, int samples, int rate, int width, int channels, const byte *data, float volume, int entityNum);
...
static void _S_AL_SanitiseVector(vec3_t v, int line)
{
	if (Q_isnan(v[0]) || Q_isnan(v[1]) || Q_isnan(v[2])) {
		Com_DPrintf(S_COLOR_YELLOW "WARNING: vector with NaN components "
				"at %s:%d -- zeroing\n", __FILE__, line);
		VectorClear(v);
	}
}
...
=================
S_AL_ScaleGain
Adapt the gain for quicker fadeout when source is too far away.
=================
*/
// Optimized attenuation calculation for better performance and sound quality
static ID_INLINE float S_AL_CalcAttenuation(float distance, float minDist, float maxDist, float rolloff) {
	if (distance <= minDist) return 1.0f;
	if (distance >= maxDist) return 0.0f;

	// Optimized inverse distance attenuation with rolloff
	// This provides more natural falloff than linear scaling
	float normalizedDist = (distance - minDist) / (maxDist - minDist);
	return 1.0f / (1.0f + rolloff * normalizedDist * normalizedDist);
}

// Batch update source positions and velocities to reduce OpenAL API calls
static void S_AL_UpdateSourcePositions(void) {
	static vec3_t lastPositions[MAX_SRC] = {0};
	static vec3_t lastVelocities[MAX_SRC] = {0};
	static float lastDistances[MAX_SRC] = {0};
	qboolean needsUpdate[MAX_SRC] = {qfalse};
	int updateCount = 0;

	// Check which sources need position updates
	for (int i = 0; i < srcCount; i++) {
		src_t *curSource = &srcList[i];

		if (!curSource->isActive || curSource->local || !curSource->isPlaying)
			continue;

		vec3_t currentPos;
		qboolean hasPosition = qfalse;

		// Get current position for this source
		if (curSource->entity >= 0 && curSource->entity < MAX_GENTITIES) {
			VectorCopy(entityList[curSource->entity].origin, currentPos);
			hasPosition = qtrue;
		} else if (!VectorCompare(curSource->loopSpeakerPos, vec3_origin)) {
			VectorCopy(curSource->loopSpeakerPos, currentPos);
			hasPosition = qtrue;
		}

		if (!hasPosition) continue;

		// Check if position changed significantly (with small epsilon to avoid micro-updates)
		if (Distance(currentPos, lastPositions[i]) > 0.1f) {
			VectorCopy(currentPos, lastPositions[i]);
			needsUpdate[i] = qtrue;
			updateCount++;

			// Pre-calculate distance for attenuation (cached for performance)
			vec3_t delta;
			VectorSubtract(currentPos, lastListenerOrigin, delta);
			lastDistances[i] = VectorLength(delta);
		}

		// For now, velocity is set to zero (can be extended for moving entities)
		if (!VectorCompare(vec3_origin, lastVelocities[i])) {
			VectorCopy(vec3_origin, lastVelocities[i]);
			// Velocity updates can be added here if needed
		}
	}

	// Batch update positions for sources that changed
	if (updateCount > 0) {
		for (int i = 0; i < srcCount; i++) {
			if (needsUpdate[i]) {
				alSource3f(srcList[i].alSource, AL_POSITION,
					lastPositions[i][0], lastPositions[i][1], lastPositions[i][2]);
			}
		}
	}
}
...
static void S_AL_ScaleGain(src_t *chksrc, vec3_t origin)
{
	...
	if (!chksrc->local) {
		// Use optimized distance calculation with SIMD where possible
		vec3_t delta;
		VectorSubtract(origin, lastListenerOrigin, delta);
		distance = VectorLength(delta);
	}

	// Use optimized attenuation calculation instead of linear scaling
	if (!chksrc->local) {
		float minDist = s_alMinDistance->value;
		float maxDist = s_alMaxDistance->value;
		float rolloff = s_alRolloff->value;

		scaleFactor = S_AL_CalcAttenuation(distance, minDist, maxDist, rolloff) * chksrc->curGain;
	} else {
		scaleFactor = chksrc->curGain;
	}

	// Only update OpenAL if the gain actually changed significantly (reduces API calls)
	if (Q_fabs(chksrc->scaleGain - scaleFactor) > 0.001f) {
		chksrc->scaleGain = scaleFactor;
		S_AL_Gain(chksrc->alSource, scaleFactor);
	}
}
...
static qboolean S_AL_HearingThroughEntity(int entityNum)
{
	...
	if (lastListenerNumber == entityNum) {
		// Detect third person by checking distance from listener to entity
		distanceSq = DistanceSquared(entityList[entityNum].origin, lastListenerOrigin);

		if (distanceSq > THIRD_PERSON_THRESHOLD_SQ)
			return qfalse; // Third person
		else
			return qtrue;  // First person
	}
	return qfalse;
}
...
static void S_AL_BufferLoad(sfxHandle_t sfx, qboolean cache)
{
	...
	while (error == AL_OUT_OF_MEMORY) {
		if (!S_AL_BufferEvict()) {
			alDeleteBuffers(1, &curSfx->buffer);
			S_AL_BufferUseDefault(sfx);
			Hunk_FreeTempMemory(data);
...
=================
S_AL_SaveLoopPos
Remove given source as loop master if it is the master and hand off master status to another source.
=================
*/
		...
		if (rmSource->priority == SRCPRI_ENTITY) {
			...
			if (!curSfx->loopActiveCnt) {
				...
				if (rmSource->isPlaying) {
					...
					// second case: all loops using this sound have stopped due to listener being out of range,
...
static void S_AL_SrcKill(srcHandle_t src)
{
	...
	if (curSource->isActive) {
		...
		srcActiveCnt--;
		// Return to free list for O(1) reallocation
		if (srcFreeCount < MAX_SRC) {
			srcFreeList[srcFreeCount++] = src;
		}
	}
	curSource->isLocked = qfalse;
	curSource->isTracking = qfalse;
...
static srcHandle_t S_AL_SrcAlloc(alSrcPriority_t priority, int entnum, int channel)
{
	...
	int weakest_pri;
	float weakest_gain;
	qboolean weakest_isplaying;
	int weakest_numloops;
	src_t *curSource;

	// Fast path: get from free list - O(1)
	if (srcFreeCount > 0) {
		empty = srcFreeList[--srcFreeCount];
		// Source should already be killed, just mark active
		srcList[empty].isActive = qtrue;
		srcActiveCnt++;
		return empty;
	}

	// Slow path: find weakest source to evict - O(n)
	weakest_time = Sys_Milliseconds();
	weakest_pri = 999;
	weakest_gain = 1000.0f;
	weakest_isplaying = qtrue;
	weakest_numloops = 0;
	...
	if (weakest >= 0) {
		...
		srcList[weakest].isActive = qtrue;
		srcActiveCnt++;
	}
...
static void S_AL_FreeStreamChannel(int stream)
{
	...
	if (streamNumBuffers[stream] > 0) {
		alDeleteBuffers(streamNumBuffers[stream], streamBuffers[stream]);
		streamNumBuffers[stream] = 0;
	}
...
=================
S_AL_RawSamples
Wrapper for simplified interface (stream 0, entityNum -1)
=================
*/
static void S_AL_RawSamples(int samples, int rate, int width, int channels, const byte *data, float volume)
{
	S_AL_RawSamplesEx(0, samples, rate, width, channels, data, volume, -1);
}

/*
=================
S_AL_RawSamplesEx
Full interface with stream and entityNum
=================
*/
static void S_AL_RawSamplesEx(int stream, int samples, int rate, int width, int channels, const byte *data, float volume, int entityNum)
{
...
static void S_AL_Update(int msec)
{
	...
	// Update master gain with underwater effect
	{
		float masterGain = s_volume->value * s_alGain->value;
		if (listenerUnderwater)
			masterGain *= UNDERWATER_GAIN_FACTOR;
	...
	// Batch update source positions for better performance
	S_AL_UpdateSourcePositions();
	...
	if (s_alDopplerSpeed->modified) {
		alSpeedOfSound(s_alDopplerSpeed->value);
...
static void S_AL_SoundInfo(void)
{
	...
	Com_Printf("  Sources:    %d\n", srcCount);
	Com_Printf("  Buffers:    %d\n", numSfx);
}
...
qboolean S_AL_Init(soundInterface_t *si)
{
	...
	Com_Printf("Initializing OpenAL sound backend...\n");

	// Cvars
	s_alPrecache = Cvar_Get("s_alPrecache", "1", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alPrecache, "Precache sounds on registration.");
	s_alGain = Cvar_Get("s_alGain", "1.0", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alGain, "OpenAL master gain multiplier.");
	s_alSources = Cvar_Get("s_alSources", "64", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alSources, "Maximum number of OpenAL sources (sound channels).");
	s_alDevice = Cvar_Get("s_alDevice", "", CVAR_ARCHIVE | CVAR_LATCH);
	Cvar_SetDescription(s_alDevice, "OpenAL device to use (empty for default).");
	s_alDopplerFactor = Cvar_Get("s_alDopplerFactor", "1.0", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alDopplerFactor, "Doppler effect intensity.");
	s_alDopplerSpeed = Cvar_Get("s_alDopplerSpeed", "9000", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alDopplerSpeed, "Speed of sound for Doppler calculations.");
	s_alMinDistance = Cvar_Get("s_alMinDistance", "120", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alMinDistance, "Minimum distance for sound attenuation.");
	...
	s_alRolloff = Cvar_Get("s_alRolloff", "2.0", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alRolloff, "Sound rolloff factor for distance attenuation.");
	s_alGraceDistance = Cvar_Get("s_alGraceDistance", "512", CVAR_ARCHIVE);
	Cvar_SetDescription(s_alGraceDistance, "Distance over which sounds fade out after s_alMaxDistance.");

	// Clear entity list
	Com_Memset(entityList, 0, sizeof(entityList));
