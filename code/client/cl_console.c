...
void Con_Init( void ) 
{
	...
	// Default scale 1, will be auto-adjusted by Con_UpdateScale after renderer init
	con_scale = Cvar_Get( "con_scale", "1", CVAR_ARCHIVE_ND );
	Cvar_CheckRange( con_scale, "0.5", "8", CV_FLOAT );
	Cvar_SetDescription( con_scale, "Console font size scale (1=FullHD, 2=2K, 4=4K/5K)." );
...
Called after renderer initialization to auto-scale console font
based on actual display resolution
================
*/
void Con_UpdateScale( void )
{
	// Only auto-scale if user hasn't set a custom value
	if ( con_scale->value == 1.0f ) {
		float scale = 1.0f;
		int width = cls.glconfig.vidWidth;
		
		if ( width >= 5120 ) {
			// 5K displays (Retina iMac, Studio Display)
			scale = 4.0f;
		} else if ( width >= 3840 ) {
			// 4K displays
			scale = 4.0f;
		...
		if ( scale > 1.0f ) {
			Cvar_SetValue( "con_scale", scale );
			Com_DPrintf( "Con_UpdateScale: auto-scaled to %.0f for %dx%d\n", 
				scale, cls.glconfig.vidWidth, cls.glconfig.vidHeight );
		}
	}
}
