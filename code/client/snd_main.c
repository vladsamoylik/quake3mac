...
void S_Update( int msec )
{
	...
	// Process pending sound load requests
	extern void S_ProcessSoundLoadQueue(void);
	S_ProcessSoundLoadQueue();
}
...
#ifdef USE_OPENAL
		s_useOpenAL = Cvar_Get( "s_useOpenAL", "1", CVAR_ARCHIVE | CVAR_LATCH );
		Cvar_SetDescription( s_useOpenAL, "Use OpenAL sound backend instead of SDL. Provides better 3D audio." );

		if ( !started && s_useOpenAL->integer ) {
			started = S_AL_Init( &si );
			if ( started ) {
				Com_Printf( "OpenAL sound backend initialized.\n" );
			} else {
				Com_Printf( "OpenAL initialization failed, falling back to SDL.\n" );
			}
		}
#endif
