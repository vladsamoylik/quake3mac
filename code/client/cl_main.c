...
void CL_StartHunkUsers( void ) {
	...
	if ( !cls.rendererStarted ) {
		...
		CL_InitRenderer();
		Con_UpdateScale();  // Auto-scale console font based on resolution
	}
...
qboolean CL_GetModeInfo( int *width, int *height, float *windowAspect, int mode, const char *modeFS, int dw, int dh, qboolean fullscreen )
{
	...
	} else if ( mode == -1 ) { // deprecated: redirect to auto-detect (-2)
		*width = dw;
		*height = dh;
		pixelAspect = r_customPixelAspect->value;
...
static void CL_InitGLimp_Cvars( void )
{
	...
	Cvar_SetDescription( r_customPixelAspect, "Enables custom aspect of the screen." );
