...
// Message buffer pooling for performance
#define MSG_POOL_SIZE 64
#define MSG_POOL_BUFFER_SIZE MAX_MSGLEN

typedef struct msg_pool_entry_s {
	byte buffer[MSG_POOL_BUFFER_SIZE];
	qboolean in_use;
	int alloc_time;
} msg_pool_entry_t;

static msg_pool_entry_t msg_pool[MSG_POOL_SIZE];
static qboolean msg_pool_initialized = qfalse;
static int msg_pool_allocs = 0;
static int msg_pool_frees = 0;
static int msg_pool_hits = 0;
static int msg_pool_misses = 0;

// Initialize message buffer pool
void MSG_InitPool(void) {
	int i;
	for (i = 0; i < MSG_POOL_SIZE; i++) {
		msg_pool[i].in_use = qfalse;
		msg_pool[i].alloc_time = 0;
	}
	msg_pool_initialized = qtrue;
	msg_pool_allocs = 0;
	msg_pool_frees = 0;
	msg_pool_hits = 0;
	msg_pool_misses = 0;
...
byte *MSG_AllocBuffer(int size) {
	...
	if (!msg_pool_initialized) {
		MSG_InitPool();
	}
	
	msg_pool_allocs++;
	
	// Check if requested size fits in pool buffer
	if (size > MSG_POOL_BUFFER_SIZE) {
		msg_pool_misses++;
	...
	for (i = 0; i < MSG_POOL_SIZE; i++) {
		if (!msg_pool[i].in_use) {
			msg_pool[i].in_use = qtrue;
			msg_pool[i].alloc_time = Com_Milliseconds();
			msg_pool_hits++;
			return msg_pool[i].buffer;
		}
	}
...
void MSG_FreeBuffer(byte *buffer) {
	...
	for (i = 0; i < MSG_POOL_SIZE; i++) {
		if (msg_pool[i].buffer == buffer) {
			msg_pool[i].in_use = qfalse;
			return;
		}
	}
	
	// Buffer was allocated from heap
	Z_Free(buffer);
}

// Get pool statistics
void MSG_GetPoolStats(int *allocs, int *frees, int *hits, int *misses) {
	if (allocs) *allocs = msg_pool_allocs;
	if (frees) *frees = msg_pool_frees;
	if (hits) *hits = msg_pool_hits;
	if (misses) *misses = msg_pool_misses;
}
