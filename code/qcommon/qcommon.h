...
void MSG_InitPool(void);
byte *MSG_AllocBuffer(int size);
void MSG_FreeBuffer(byte *buffer);
void MSG_GetPoolStats(int *allocs, int *frees, int *hits, int *misses);
...
typedef struct {
	...
	int networkBytesIn;
	int networkBytesOut;
	int networkPacketsIn;
	int networkPacketsOut;
} perfStats_t;
...
// Generic memory pool allocator  
typedef struct mem_pool_s mem_pool_t;
void MemPool_Init(void);
mem_pool_t *MemPool_Create(const char *name, int object_size, int initial_count);
void *MemPool_Alloc(mem_pool_t *pool);
void MemPool_Free(mem_pool_t *pool, void *ptr);
void MemPool_GetStats(mem_pool_t *pool, int *allocated, int *in_use, int *peak);
...
typedef enum {
	LOAD_PRIORITY_CRITICAL = 0,
	LOAD_PRIORITY_HIGH = 1,
	LOAD_PRIORITY_NORMAL = 2,
	LOAD_PRIORITY_LOW = 3,
	LOAD_PRIORITY_PRELOAD = 4
} loadPriority_t;
...
// Resource preloading for next map
