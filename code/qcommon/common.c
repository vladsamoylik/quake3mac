...
#define DEF_COMHUNKMEGS		256  // Increased for 4K/5K screenshots
#endif
...
static perfStats_t perfStats;
static qboolean perfStats_initialized = qfalse;

// Memory leak detection (enabled with developer mode)
#ifdef _DEBUG
#define TRACK_MEMORY_LEAKS
#endif

#ifdef TRACK_MEMORY_LEAKS
#define MAX_MEMORY_TRACKS 8192
...
static mem_track_t mem_tracks[MAX_MEMORY_TRACKS];
static int mem_track_count = 0;
static int total_allocated = 0;
static int total_freed = 0;
static qboolean mem_tracking_enabled = qfalse;
#endif

// Initialize performance statistics
void Com_InitPerfStats(void) {
	Com_Memset(&perfStats, 0, sizeof(perfStats));
	perfStats_initialized = qtrue;
}

// Get current performance statistics
void Com_GetPerfStats(perfStats_t *stats) {
	if (!perfStats_initialized) {
		Com_InitPerfStats();
	}
...
void Com_UpdateNetworkStats(int bytesIn, int bytesOut, int packetsIn, int packetsOut) {
	if (!perfStats_initialized) {
		Com_InitPerfStats();
	}
	
	perfStats.networkBytesIn += bytesIn;
	perfStats.networkBytesOut += bytesOut;
	perfStats.networkPacketsIn += packetsIn;
	perfStats.networkPacketsOut += packetsOut;
}

// Export performance statistics to CSV format
void Com_ExportPerfStatsCSV(const char *filename) {
	fileHandle_t f;
	char line[1024];
	
	if (!perfStats_initialized) {
		Com_Printf("Performance stats not initialized\n");
		return;
	}
	...
	// Write CSV header
	Com_sprintf(line, sizeof(line), "Metric,Value\n");
	FS_Write(line, strlen(line), f);
	
	// Write CPU time stats
	Com_sprintf(line, sizeof(line), "CPU Time Total (ms),%d\n", perfStats.cpuTimeTotal);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "CPU Time Renderer (ms),%d\n", perfStats.cpuTimeRenderer);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "CPU Time Game (ms),%d\n", perfStats.cpuTimeGame);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "CPU Time Network (ms),%d\n", perfStats.cpuTimeNetwork);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "CPU Time Sound (ms),%d\n", perfStats.cpuTimeSound);
	FS_Write(line, strlen(line), f);
	
	// Write memory stats
	Com_sprintf(line, sizeof(line), "Memory Total (bytes),%d\n", perfStats.memoryUsedTotal);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "Memory Hunk (bytes),%d\n", perfStats.memoryUsedHunk);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "Memory Zone (bytes),%d\n", perfStats.memoryUsedZone);
	FS_Write(line, strlen(line), f);
	
	// Write network stats
	Com_sprintf(line, sizeof(line), "Network Bytes In,%d\n", perfStats.networkBytesIn);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "Network Bytes Out,%d\n", perfStats.networkBytesOut);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "Network Packets In,%d\n", perfStats.networkPacketsIn);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "Network Packets Out,%d\n", perfStats.networkPacketsOut);
	FS_Write(line, strlen(line), f);
	
	FS_FCloseFile(f);
	Com_Printf("Performance statistics exported to %s\n", filename);
}

#ifdef TRACK_MEMORY_LEAKS
// Enable memory leak tracking
void Com_EnableMemoryTracking(qboolean enable) {
	mem_tracking_enabled = enable;
	if (enable) {
		Com_Printf("Memory leak tracking enabled\n");
		mem_track_count = 0;
		total_allocated = 0;
		total_freed = 0;
	}
...
void Com_TrackAlloc(void *ptr, int size, const char *file, int line) {
	int i;
	
	if (!mem_tracking_enabled || !ptr) {
		return;
	}
	
	// Find free slot
	for (i = 0; i < MAX_MEMORY_TRACKS; i++) {
		if (!mem_tracks[i].allocated) {
			mem_tracks[i].ptr = ptr;
			mem_tracks[i].size = size;
			mem_tracks[i].file = file;
			mem_tracks[i].line = line;
			mem_tracks[i].allocated = 1;
			mem_track_count++;
	...
	Com_Printf("Warning: Memory tracking table full\n");
}

// Track memory free
void Com_TrackFree(void *ptr) {
	int i;
	...
	for (i = 0; i < MAX_MEMORY_TRACKS; i++) {
		if (mem_tracks[i].allocated && mem_tracks[i].ptr == ptr) {
			total_freed += mem_tracks[i].size;
			mem_tracks[i].allocated = 0;
			mem_track_count--;
			return;
		}
	}
...
// Report memory leaks
void Com_ReportMemoryLeaks(void) {
	int i, leak_count = 0;
	int total_leaked = 0;
	
	if (!mem_tracking_enabled) {
		Com_Printf("Memory leak tracking is disabled\n");
		return;
	}
	
	Com_Printf("\n===== Memory Leak Report =====\n");
	Com_Printf("Total allocated: %d bytes\n", total_allocated);
	Com_Printf("Total freed: %d bytes\n", total_freed);
	Com_Printf("Active allocations: %d\n", mem_track_count);
	
	if (mem_track_count > 0) {
		Com_Printf("\nLEAKED MEMORY:\n");
		for (i = 0; i < MAX_MEMORY_TRACKS; i++) {
			if (mem_tracks[i].allocated) {
				Com_Printf("  %d bytes at %p from %s:%d\n",
					mem_tracks[i].size,
					mem_tracks[i].ptr,
					mem_tracks[i].file ? mem_tracks[i].file : "unknown",
					mem_tracks[i].line);
				total_leaked += mem_tracks[i].size;
				leak_count++;
			}
		}
		Com_Printf("\nTotal: %d leaks, %d bytes leaked\n", leak_count, total_leaked);
	} else {
...
void Com_EnableMemoryTracking(qboolean enable) { (void)enable; }
void Com_TrackAlloc(void *ptr, int size, const char *file, int line) { (void)ptr; (void)size; (void)file; (void)line; }
void Com_TrackFree(void *ptr) { (void)ptr; }
void Com_ReportMemoryLeaks(void) { Com_Printf("Memory leak tracking not available in release builds\n"); }
#endif

// Export performance statistics to JSON format
void Com_ExportPerfStatsJSON(const char *filename) {
	fileHandle_t f;
	char line[1024];
	...
	// CPU stats
	Com_sprintf(line, sizeof(line), "  \"cpu\": {\n");
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"total_ms\": %d,\n", perfStats.cpuTimeTotal);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"renderer_ms\": %d,\n", perfStats.cpuTimeRenderer);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"game_ms\": %d,\n", perfStats.cpuTimeGame);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"network_ms\": %d,\n", perfStats.cpuTimeNetwork);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"sound_ms\": %d\n", perfStats.cpuTimeSound);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "  },\n");
	FS_Write(line, strlen(line), f);
	
	// Memory stats
	Com_sprintf(line, sizeof(line), "  \"memory\": {\n");
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"total_bytes\": %d,\n", perfStats.memoryUsedTotal);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"hunk_bytes\": %d,\n", perfStats.memoryUsedHunk);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"zone_bytes\": %d\n", perfStats.memoryUsedZone);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "  },\n");
	FS_Write(line, strlen(line), f);
	
	// Network stats
	Com_sprintf(line, sizeof(line), "  \"network\": {\n");
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"bytes_in\": %d,\n", perfStats.networkBytesIn);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"bytes_out\": %d,\n", perfStats.networkBytesOut);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"packets_in\": %d,\n", perfStats.networkPacketsIn);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "    \"packets_out\": %d\n", perfStats.networkPacketsOut);
	FS_Write(line, strlen(line), f);
	Com_sprintf(line, sizeof(line), "  }\n");
	FS_Write(line, strlen(line), f);
...
void Com_Init( char *commandLine ) {
	...
	// Initialize message buffer pool for network performance
	MSG_InitPool();
...
void Com_Frame( qboolean noDelay ) {
	...
	// Process async file loading requests (up to 4 per frame)
	FS_ProcessAsyncRequests(4);
