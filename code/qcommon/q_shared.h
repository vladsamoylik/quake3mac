...
// SIMD optimized functions for Apple Silicon
#ifdef __ARM_NEON
#include <arm_neon.h>

// NEON helper functions
static ID_INLINE float32x4_t VectorLoad(const vec3_t v) {
    return vld1q_f32(v);
}

static ID_INLINE void VectorStore(vec3_t v, float32x4_t vec) {
    vst1q_f32(v, vec);
}

// NEON optimized vector operations
static ID_INLINE vec_t VectorLengthNEON(const vec3_t v) {
    float32x4_t vec = VectorLoad(v);
...
static ID_INLINE vec_t VectorLengthSquaredNEON(const vec3_t v) {
    float32x4_t vec = VectorLoad(v);
    float32x4_t sq = vmulq_f32(vec, vec);
    float32x2_t sum = vadd_f32(vget_low_f32(sq), vget_high_f32(sq));
    sum = vpadd_f32(sum, sum);
...
static ID_INLINE void VectorNormalizeFastNEON(vec3_t v) {
    float32x4_t vec = VectorLoad(v);
    float32x4_t sq = vmulq_f32(vec, vec);
    float32x2_t sum = vadd_f32(vget_low_f32(sq), vget_high_f32(sq));
    sum = vpadd_f32(sum, sum);
    ...
    inv_len = vmul_f32(vrsqrts_f32(vmul_f32(sum, inv_len), inv_len), inv_len);

    float32x4_t scale = vcombine_f32(inv_len, inv_len);
    vec = vmulq_f32(vec, scale);
    VectorStore(v, vec);
}

static ID_INLINE vec_t DotProductNEON(const vec3_t v1, const vec3_t v2) {
    float32x4_t a = VectorLoad(v1);
    float32x4_t b = VectorLoad(v2);
...
static ID_INLINE void VectorAddNEON(const vec3_t a, const vec3_t b, vec3_t out) {
    float32x4_t va = VectorLoad(a);
    float32x4_t vb = VectorLoad(b);
    float32x4_t result = vaddq_f32(va, vb);
    VectorStore(out, result);
}

static ID_INLINE void VectorSubtractNEON(const vec3_t a, const vec3_t b, vec3_t out) {
    float32x4_t va = VectorLoad(a);
    float32x4_t vb = VectorLoad(b);
...
static ID_INLINE void VectorScaleNEON(const vec3_t v, float scale, vec3_t out) {
    float32x4_t vec = VectorLoad(v);
    float32x4_t s = vdupq_n_f32(scale);
    float32x4_t result = vmulq_f32(vec, s);
    VectorStore(out, result);
}
...
static ID_INLINE vec_t VectorLength( const vec3_t v ) {
	// Temporarily disable NEON due to potential precision issues
	// #ifdef __ARM_NEON
	//	return VectorLengthNEON(v);
	// #else
...
#ifdef __ARM_NEON
	return VectorLengthSquaredNEON(v);
#else
