...
// File cache system for improved I/O performance
#define FILE_CACHE_SIZE 128
#define FILE_CACHE_MAX_FILE_SIZE (512 * 1024) // 512KB max cached file size
#define FILE_CACHE_MAX_MEMORY (16 * 1024 * 1024) // 16MB max cache memory

typedef struct file_cache_entry_s {
...
static file_cache_entry_t file_cache[FILE_CACHE_SIZE];
static qboolean file_cache_initialized = qfalse;
static int file_cache_frame = 0;
static int file_cache_memory = 0;
static int file_cache_hits = 0;
static int file_cache_misses = 0;

// Initialize file cache
static void FS_InitFileCache(void) {
	int i;
	for (i = 0; i < FILE_CACHE_SIZE; i++) {
		file_cache[i].filename[0] = '\0';
		file_cache[i].data = NULL;
		file_cache[i].size = 0;
		file_cache[i].last_used_time = 0;
		file_cache[i].in_use = qfalse;
	}
	file_cache_initialized = qtrue;
	file_cache_frame = 0;
	file_cache_memory = 0;
	file_cache_hits = 0;
	file_cache_misses = 0;
...
// Find file in cache
static file_cache_entry_t *FS_FindCachedFile(const char *filename) {
	int i;
	...
	for (i = 0; i < FILE_CACHE_SIZE; i++) {
		if (file_cache[i].in_use && Q_stricmp(file_cache[i].filename, filename) == 0) {
			file_cache[i].last_used_time = file_cache_frame++;
			file_cache_hits++;
			return &file_cache[i];
		}
	}
...
// Add file to cache
static void FS_AddToFileCache(const char *filename, byte *data, int size) {
	int i;
	int oldest_index = 0;
	int oldest_time = file_cache[0].last_used_time;
	
	if (!file_cache_initialized || size > FILE_CACHE_MAX_FILE_SIZE) {
		return;
	}
	
	// Check if we need to free memory
	while (file_cache_memory + size > FILE_CACHE_MAX_MEMORY) {
		// Find and remove oldest entry
		oldest_index = 0;
		oldest_time = file_cache[0].last_used_time;
		
		for (i = 1; i < FILE_CACHE_SIZE; i++) {
			if (file_cache[i].in_use && file_cache[i].last_used_time < oldest_time) {
				oldest_time = file_cache[i].last_used_time;
				oldest_index = i;
			}
		}
		
		if (file_cache[oldest_index].in_use) {
			file_cache_memory -= file_cache[oldest_index].size;
			Z_Free(file_cache[oldest_index].data);
			file_cache[oldest_index].in_use = qfalse;
			file_cache[oldest_index].data = NULL;
		} else {
			break; // No more entries to free
		}
	}
	
	// Find free slot
	for (i = 0; i < FILE_CACHE_SIZE; i++) {
		if (!file_cache[i].in_use) {
			Q_strncpyz(file_cache[i].filename, filename, sizeof(file_cache[i].filename));
			file_cache[i].data = Z_Malloc(size + 1);
			Com_Memcpy(file_cache[i].data, data, size);
			file_cache[i].data[size] = '\0';
			file_cache[i].size = size;
			file_cache[i].last_used_time = file_cache_frame++;
			file_cache[i].in_use = qtrue;
			file_cache_memory += size;
			return;
		}
	}
}
...
void FS_GetCacheStats(int *hits, int *misses, int *memory) {
	if (hits) *hits = file_cache_hits;
	if (misses) *misses = file_cache_misses;
	if (memory) *memory = file_cache_memory;
}

// Async file loading system (queue-based, processed in main loop)
#define MAX_ASYNC_REQUESTS 64

typedef void (*asyncLoadCallback_t)(const char *filename, void *buffer, int size, void *userdata);

typedef struct {
	char filename[MAX_QPATH];
	asyncLoadCallback_t callback;
	void *userdata;
	qboolean pending;
	qboolean completed;
	void *buffer;
...
static asyncLoadRequest_t async_requests[MAX_ASYNC_REQUESTS];
static int async_request_count = 0;
static qboolean async_system_initialized = qfalse;

// Initialize async loading system
static void FS_InitAsyncSystem(void) {
	int i;
	for (i = 0; i < MAX_ASYNC_REQUESTS; i++) {
		async_requests[i].pending = qfalse;
		async_requests[i].completed = qfalse;
		async_requests[i].buffer = NULL;
	}
	async_system_initialized = qtrue;
	async_request_count = 0;
}

// Queue async file load request with priority
int FS_AsyncReadFileWithPriority(const char *filename, asyncLoadCallback_t callback, void *userdata, loadPriority_t priority) {
	int i;
	...
	// Find free slot
	for (i = 0; i < MAX_ASYNC_REQUESTS; i++) {
		if (!async_requests[i].pending && !async_requests[i].completed) {
			Q_strncpyz(async_requests[i].filename, filename, sizeof(async_requests[i].filename));
			async_requests[i].callback = callback;
			async_requests[i].userdata = userdata;
			async_requests[i].pending = qtrue;
			async_requests[i].completed = qfalse;
			async_requests[i].buffer = NULL;
			async_requests[i].size = 0;
			async_requests[i].priority = priority;
			async_requests[i].request_time = Com_Milliseconds();
			async_request_count++;
			return i;
		}
	}
...
// Queue async file load request (normal priority)
int FS_AsyncReadFile(const char *filename, asyncLoadCallback_t callback, void *userdata) {
	return FS_AsyncReadFileWithPriority(filename, callback, userdata, LOAD_PRIORITY_NORMAL);
}

// Process async file loading requests (call from main loop)
// Processes highest priority requests first
void FS_ProcessAsyncRequests(int maxRequests) {
	int i, processed = 0;
	int best_index;
	int best_priority;
	...
	while (processed < maxRequests) {
		best_index = -1;
		best_priority = LOAD_PRIORITY_PRELOAD + 1;
		
		// Find highest priority pending request
		for (i = 0; i < MAX_ASYNC_REQUESTS; i++) {
			if (async_requests[i].pending && !async_requests[i].completed) {
				if (async_requests[i].priority < best_priority) {
					best_priority = async_requests[i].priority;
					best_index = i;
				}
			}
		}
		...
		// Load file synchronously (in future could be threaded)
		async_requests[i].size = FS_ReadFile(async_requests[i].filename, &async_requests[i].buffer);
		async_requests[i].pending = qfalse;
		async_requests[i].completed = qtrue;
		
		// Call callback
		if (async_requests[i].callback) {
			async_requests[i].callback(
				async_requests[i].filename,
				async_requests[i].buffer,
				async_requests[i].size,
				async_requests[i].userdata
			);
		}
		
		// Free buffer if loaded
		if (async_requests[i].buffer) {
			FS_FreeFile(async_requests[i].buffer);
			async_requests[i].buffer = NULL;
		}
...
// Get number of pending async requests
int FS_GetAsyncRequestCount(void) {
	return async_request_count;
}

// Resource preloading system for next map
#define MAX_PRELOAD_FILES 128

static char preload_files[MAX_PRELOAD_FILES][MAX_QPATH];
static int preload_count = 0;
static qboolean preload_enabled = qfalse;

// Add file to preload list
void FS_AddToPreloadList(const char *filename) {
	if (preload_count >= MAX_PRELOAD_FILES) {
		return;
	}
	
	Q_strncpyz(preload_files[preload_count], filename, sizeof(preload_files[preload_count]));
	preload_count++;
}

// Clear preload list
void FS_ClearPreloadList(void) {
	preload_count = 0;
}

// Start preloading files
void FS_StartPreload(void) {
	int i;
	
	if (!preload_enabled || preload_count == 0) {
		return;
	}
	
	// Queue all files with PRELOAD priority
	for (i = 0; i < preload_count; i++) {
		FS_AsyncReadFileWithPriority(preload_files[i], NULL, NULL, LOAD_PRIORITY_PRELOAD);
	}
	
	Com_Printf("Preloading %d files for next map\n", preload_count);
	preload_count = 0;
}

// Enable/disable preloading
void FS_EnablePreload(qboolean enable) {
	preload_enabled = enable;
	if (enable) {
		Com_Printf("Resource preloading enabled\n");
	} else {
		Com_Printf("Resource preloading disabled\n");
	}
}
...
// ZIP file lookup cache for recently accessed files
#define ZIP_LOOKUP_CACHE_SIZE 256
...
static zip_lookup_cache_t zip_lookup_cache[ZIP_LOOKUP_CACHE_SIZE];
static int zip_lookup_frame = 0;
static qboolean zip_lookup_initialized = qfalse;
static int zip_lookup_hits = 0;
static int zip_lookup_misses = 0;
...
// ZIP lookup cache functions

// Initialize ZIP lookup cache
static void FS_InitZIPLookupCache(void) {
	int i;
	for (i = 0; i < ZIP_LOOKUP_CACHE_SIZE; i++) {
		zip_lookup_cache[i].filename[0] = '\0';
		zip_lookup_cache[i].pack = NULL;
		zip_lookup_cache[i].file = NULL;
		zip_lookup_cache[i].last_used = 0;
		zip_lookup_cache[i].valid = qfalse;
	}
	zip_lookup_initialized = qtrue;
	zip_lookup_frame = 0;
	zip_lookup_hits = 0;
	zip_lookup_misses = 0;
}

// Find file in ZIP lookup cache
static fileInPack_t *FS_FindInZIPCache(const char *filename, pack_t **out_pack) {
	uint32_t hash;
	unsigned long i;
	
	if (!zip_lookup_initialized) {
		FS_InitZIPLookupCache();
	}
	
	// Simple hash for cache index
	for (i = 0, hash = 0; filename[i]; i++) {
		hash = hash * 31 + tolower(filename[i]);
	}
	hash = hash % ZIP_LOOKUP_CACHE_SIZE;
	
	// Check cache entry
	if (zip_lookup_cache[hash].valid && 
		Q_stricmp(zip_lookup_cache[hash].filename, filename) == 0) {
		zip_lookup_cache[hash].last_used = zip_lookup_frame++;
		zip_lookup_hits++;
		if (out_pack) *out_pack = zip_lookup_cache[hash].pack;
		return zip_lookup_cache[hash].file;
	}
...
// Add file to ZIP lookup cache
static void FS_AddToZIPCache(const char *filename, pack_t *pack, fileInPack_t *file) {
	uint32_t hash;
	unsigned long i;
	...
	Q_strncpyz(zip_lookup_cache[hash].filename, filename, sizeof(zip_lookup_cache[hash].filename));
	zip_lookup_cache[hash].pack = pack;
	zip_lookup_cache[hash].file = file;
	zip_lookup_cache[hash].last_used = zip_lookup_frame++;
	zip_lookup_cache[hash].valid = qtrue;
}

// Get ZIP lookup cache statistics
void FS_GetZIPCacheStats(int *hits, int *misses) {
	if (hits) *hits = zip_lookup_hits;
	if (misses) *misses = zip_lookup_misses;
}
...
int FS_FOpenFileRead( const char *filename, fileHandle_t *file, qboolean uniqueFILE ) {
	...
	//
	// Check ZIP lookup cache first for fast path
	//
	pakFile = FS_FindInZIPCache( filename, &pak );
	if ( pakFile != NULL && FS_PakIsPure( pak ) ) {
		// Found in cache!
	...
	for ( search = fs_searchpaths ; search ; search = search->next ) {
		...
		if ( search->pack && search->pack->hashTable[ (hash = fullHash & (search->pack->hashSize-1)) ] ) {
			...
			do {
				// case and separator insensitive comparisons
				if ( !FS_FilenameCompare( pakFile->name, filename ) ) {
					// found it! Add to cache for next time
					FS_AddToZIPCache( filename, pak, pakFile );
					return FS_OpenFileInPak( file, pak, pakFile, uniqueFILE );
				}
				pakFile = pakFile->next;
			} while ( pakFile != NULL );
...
int FS_ReadFile( const char *qpath, void **buffer ) {
	...
	// Check cache first
	if (buffer != NULL) {
		cached = FS_FindCachedFile(qpath);
		if (cached != NULL) {
			buf = Hunk_AllocateTempMemory(cached->size + 1);
			Com_Memcpy(buf, cached->data, cached->size + 1);
			*buffer = buf;
			fs_loadCount++;
			fs_loadStack++;
			return cached->size;
	...
	*buffer = buf;
	...
	FS_AddToFileCache(qpath, buf, len);
...
void FS_InitFilesystem( void ) {
	// Initialize file cache for improved I/O performance
	FS_InitFileCache();
