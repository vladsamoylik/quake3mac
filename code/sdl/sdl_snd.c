...
qboolean SNDDMA_Init( void )
{
	...
	if ( desired.freq == 0 )
		desired.freq = 48000;  // Professional quality default
	...
	{
		// Optimized for low latency on modern systems (Apple Silicon)
		if (desired.freq <= 11025)
			desired.samples = 256;   // ~23ms
		else if (desired.freq <= 22050)
			desired.samples = 512;   // ~23ms
		else if (desired.freq <= 44100)
			desired.samples = 512;   // ~11ms
		else
			desired.samples = 512;   // ~10ms for 48kHz - low latency!
	}
