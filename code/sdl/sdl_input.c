...
static void IN_GobbleMouseEvents( void )
{
	...
	// Gobble any mouse events, including button clicks
	SDL_PumpEvents();

	// Clear motion and wheel events
	while( ( val = SDL_PeepEvents( dummy, ARRAY_LEN( dummy ), SDL_GETEVENT,
		SDL_MOUSEMOTION, SDL_MOUSEWHEEL ) ) > 0 ) { }

	// Clear button events separately (they're not in the same range)
	while( ( val = SDL_PeepEvents( dummy, ARRAY_LEN( dummy ), SDL_GETEVENT,
...
static void IN_ActivateMouse( void )
{
	...
	if ( !mouseActive )
	{
...
#ifdef MACOS_X
		// macOS workaround: Set grab before relative mode to avoid event loop deadlock
		SDL_SetWindowGrab( SDL_window, SDL_TRUE );
		SDL_PumpEvents(); // Process pending events to avoid freeze
		...
		// Clear any synthetic events generated by mode changes and warp
		IN_GobbleMouseEvents();
