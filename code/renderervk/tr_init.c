...
static void InitOpenGL( void )
{
	...
	if ( glConfig.vidWidth == 0 )
	{
		...
		// Update window dimensions after VKimp_Init sets glConfig.vidWidth/vidHeight
		// This is especially important on macOS with HiDPI where GetDrawableSize
		// returns physical pixels that may differ from logical window size
...
==================
*/
void VK_CheckErrors( void ) {
	// Vulkan doesn't use OpenGL error checking
}
...
Screenshots are saved to fs_homepath + fs_gamedir/screenshots/
Filename format: shot-YYYYMMDD-HHMMSS.ext (with counter suffix if file exists)
...
static void R_ScreenShot_f( void ) {
	...
	if ( Q_stricmp( ri.Cmd_Argv(0), "screenshotTGA" ) == 0 ) {
		...
		// Default to JPEG (screenshot and screenshotJPEG commands)
...
static void R_Register( void )
{
	...
	ri.Cvar_SetDescription( r_ignoreGLErrors, "Ignore renderer errors (legacy cvar, not used in Vulkan)." );
	...
	r_screenshotJpegQuality = ri.Cvar_Get( "r_screenshotJpegQuality", "98", CVAR_ARCHIVE_ND );
	ri.Cvar_SetDescription( r_screenshotJpegQuality, "Controls quality of JPEG screenshots (1-100, default 100 for lossless quality)." );
	...
	r_bloom_threshold_mode = ri.Cvar_Get( "r_bloom_threshold_mode", "2", CVAR_ARCHIVE_ND );
	...
	r_bloom_modulate = ri.Cvar_Get( "r_bloom_modulate", "2", CVAR_ARCHIVE_ND );
	...
	// Remove CVAR_LATCH to allow bloom to be changed without vid_restart (fixes menu crash on startup)
	r_bloom->flags &= ~CVAR_LATCH;

	r_edr = ri.Cvar_Get( "r_edr", "0", CVAR_ARCHIVE_ND | CVAR_LATCH );
	ri.Cvar_CheckRange( r_edr, "0", "1", CV_INTEGER );
	ri.Cvar_SetDescription( r_edr, "Enable macOS EDR (Extended Dynamic Range) output on Apple XDR displays via CAMetalLayer. Requires \\r_fbo 1 and Vulkan/MoltenVK. Uses float swapchain when available." );
	ri.Cvar_SetGroup( r_edr, CVG_RENDERER );

	r_edr_toneMapping = ri.Cvar_Get( "r_edr_toneMapping", "0", CVAR_ARCHIVE_ND );
	ri.Cvar_CheckRange( r_edr_toneMapping, "0", "2", CV_INTEGER );
	ri.Cvar_SetDescription( r_edr_toneMapping, "Tone mapping for EDR/HDR output:\n 0: Linear/gamma (default)\n 1: PQ (HDR10-style, cinematic)\n 2: ACES Filmic (alternative cinematic)" );
	ri.Cvar_SetGroup( r_edr_toneMapping, CVG_RENDERER );

	r_ext_multisample = ri.Cvar_Get( "r_ext_multisample", "4", CVAR_ARCHIVE_ND | CVAR_LATCH );
