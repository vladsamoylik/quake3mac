...
image_t *R_CreateImage( const char *name, const char *name2, byte *pic, int width, int height, imgFlags_t flags ) {
	...
	image->width = width;
	image->height = height;
	image->lastUsedTime = tr.frameCount;
...
/*
===============
R_CleanupLRUTextures

Clean up least recently used textures to free memory
===============
*/
void R_CleanupLRUTextures( void ) {
	int hash;
	image_t *image, *prev, *next;
	int oldestTime = tr.frameCount;
	image_t *oldestImage = NULL;
	int textureMemoryFreed = 0;
	const int MAX_TEXTURE_AGE = 32768; // 32768 frames = ~8 minutes at 60fps

	// Find textures that haven't been used for a while
	for ( hash = 0; hash < FILE_HASH_SIZE; hash++ ) {
		for ( image = hashTable[hash]; image; image = image->next ) {
			// Skip special images and recently used textures
			if ( image->imgName[0] == '*' ||
				 (tr.frameCount - image->lastUsedTime) < MAX_TEXTURE_AGE ) {
				continue;
			}

			// Find the oldest texture
			if ( image->lastUsedTime < oldestTime ) {
				oldestTime = image->lastUsedTime;
				oldestImage = image;
			}
		}
	}

	// Free the oldest texture if found
	if ( oldestImage ) {
		// Remove from hash table
		for ( hash = 0; hash < FILE_HASH_SIZE; hash++ ) {
			prev = NULL;
			for ( image = hashTable[hash]; image; image = next ) {
				next = image->next;
				if ( image == oldestImage ) {
					...
					// For now, just mark as unused - full cleanup requires Vulkan context
					// TODO: Implement proper Vulkan resource cleanup in render thread
					ri.Printf( PRINT_DEVELOPER, "LRU marked texture for cleanup: %s (age: %d frames)\n",
							  oldestImage->imgName, tr.frameCount - oldestImage->lastUsedTime );

					// Mark as not recently used (but don't actually free yet)
					oldestImage->lastUsedTime = tr.frameCount - MAX_TEXTURE_AGE + 1;

					break;
				}
				prev = image;
			}
			if ( oldestImage == NULL ) break; // found and freed
		}
	}

	if ( textureMemoryFreed > 0 ) {
		ri.Printf( PRINT_DEVELOPER, "LRU texture cleanup: freed %d KB\n", textureMemoryFreed / 1024 );
	}
}
