...
// Global flag to track first initialization (before UI loads)
// Used to delay bloom resource creation until UI is fully loaded
static qboolean vk_first_init = qtrue;
...
static void vk_create_render_passes( void )
{
	...
	// On first initialization (before UI loads), don't create bloom resources to prevent menu crash
	// Bloom will be enabled after UI is fully loaded via vid_restart
	qboolean use_bloom = r_bloom->integer && !vk_first_init;
	qboolean create_bloom_passes = r_bloom->integer && !vk_first_init;
...
static qboolean vk_select_surface_format( VkPhysicalDevice physical_device, VkSurfaceKHR surface )
{
	...
	// EDR path: try to select float16 swapchain format
	if ( r_edr && r_edr->integer && r_fbo && r_fbo->integer ) {
		for ( uint32_t i = 0; i < format_count; i++ ) {
			// Accept float16 with any colorspace (Linear or Extended are both fine)
			if ( candidates[i].format == VK_FORMAT_R16G16B16A16_SFLOAT ) {
				vk.base_format = candidates[i];
				vk.present_format = candidates[i];
				ri.Printf( PRINT_ALL, "...EDR: selected VK_FORMAT_R16G16B16A16_SFLOAT (colorspace=%d)\n",
				           (int)candidates[i].colorSpace );
				ri.Free( candidates );
				return qtrue;
			}
		}

		// Fallback: try 10-bit format for some EDR-like behavior
		ri.Printf( PRINT_WARNING, "...EDR: float16 not available, trying 10-bit format\n" );
		for ( uint32_t i = 0; i < format_count; i++ ) {
			if ( candidates[i].format == VK_FORMAT_A2B10G10R10_UNORM_PACK32 ||
			     candidates[i].format == VK_FORMAT_A2R10G10B10_UNORM_PACK32 ) {
				vk.base_format = candidates[i];
				vk.present_format = candidates[i];
				ri.Printf( PRINT_ALL, "...EDR fallback: selected 10-bit format %s\n",
				           vk_format_string( candidates[i].format ) );
				ri.Free( candidates );
				return qtrue;
			}
		}

		ri.Printf( PRINT_WARNING, "...EDR: no suitable format found, falling back to SDR\n" );
	}
...
static void setup_surface_formats( VkPhysicalDevice physical_device )
{
	...
	// Choose internal rendering color format
	if ( r_edr && r_edr->integer && r_fbo && r_fbo->integer ) {
		// EDR requires float internal buffer
		vk.color_format = VK_FORMAT_R16G16B16A16_SFLOAT;
		ri.Printf( PRINT_ALL, "...EDR: internal color buffer = R16G16B16A16_SFLOAT\n" );
	} else {
	...
	// Check EDR compatibility after format selection
	if ( r_edr && r_edr->integer ) {
		if ( !r_fbo || !r_fbo->integer ) {
			ri.Printf( PRINT_ERROR, "EDR requires r_fbo 1. Disabling EDR.\n" );
			ri.Cvar_Set( "r_edr", "0" );
		} else if ( vk.present_format.format != VK_FORMAT_R16G16B16A16_SFLOAT &&
		            vk.present_format.format != VK_FORMAT_A2B10G10R10_UNORM_PACK32 &&
		            vk.present_format.format != VK_FORMAT_A2R10G10B10_UNORM_PACK32 ) {
			ri.Printf( PRINT_WARNING, "EDR: swapchain format %s may not support extended range (expected float16 or 10-bit)\n",
...
void vk_update_attachment_descriptors( void ) {
	...
	if ( vk.color_image_view )
	{
		...
		// bloom images - only update if they were created (not on first init)
		if ( r_bloom->integer && vk.bloom_image_view[0] != VK_NULL_HANDLE )
		{
			uint32_t i;
...
void vk_init_descriptors( void )
{
	...
	if ( vk.color_image_view )
	{
		...
		// Only allocate bloom descriptors if bloom resources were created (not on first init)
		if ( r_bloom->integer && !vk_first_init )
		{
			for ( i = 0; i < ARRAY_LEN( vk.bloom_image_descriptor ); i++ )
			{
...
void vk_update_post_process_pipelines( void )
{
	if ( vk.fboActive ) {
		...
		// Only create bloom pipelines if render passes were created (not on first init)
		if ( r_bloom->integer && vk.render_pass.bloom_extract != VK_NULL_HANDLE ) {
...
static void vk_create_attachments( void )
{
	...
	if ( vk.fboActive ) {
		...
		// bloom - only create on subsequent initializations (after UI loads)
		if ( r_bloom->integer && !vk_first_init ) {
			uint32_t width = gls.captureWidth;
			uint32_t height = gls.captureHeight;
	...
	create_depth_attachment( glConfig.vidWidth, glConfig.vidHeight, vkSamples, &vk.depth_image, &vk.depth_image_view,
		(vk.fboActive && r_bloom->integer && !vk_first_init) ? qfalse : qtrue );
...
static void vk_create_framebuffers( void )
{
	...
	if ( vk.fboActive )
	{
		...
		// Only create bloom framebuffers if render passes were created (not on first init)
		if ( r_bloom->integer && vk.render_pass.bloom_extract != VK_NULL_HANDLE )
		{
			uint32_t width = gls.captureWidth;
			uint32_t height = gls.captureHeight;
...
static void vk_restart_swapchain( const char *funcname, VkResult res )
{
	...
	// Reset vk_first_init flag after all resources are created
	// This allows bloom to work on subsequent calls (e.g., vid_restart)
	// On first init, bloom resources are not created to prevent menu crash
	// After UI loads and vid_restart is called, bloom will be enabled
	if ( vk_first_init ) {
		vk_first_init = qfalse;
	}
}
...
void vk_initialize( void )
{
	...
	// On macOS in fullscreen, ensure blit offsets are zero to avoid border frame
	// This is important because HiDPI scaling can cause size mismatches that
	// incorrectly set blit offsets, creating a visible border
#ifdef MACOS_X
	...
	// do early texture mode setup to avoid redundant descriptor updates in VK_SetDefaultState()
...
void vk_create_post_process_pipeline( int program_index, uint32_t width, uint32_t height )
{
	...
	if ( r_edr && r_edr->integer ) {
		// EDR path: linear output (gamma=1.0), but allow r_gamma as brightness/exposure multiplier
		frag_spec_data.gamma = 1.0f;
		frag_spec_data.overbright = r_gamma->value * (float)(1 << tr.overbrightBits);
		frag_spec_data.dither = 0;
		// Use tone mapping if enabled (PQ for HDR10-style, ACES for alternative)
		frag_spec_data.toneMapping = r_edr_toneMapping ? r_edr_toneMapping->integer : 0;
		ri.Printf( PRINT_DEVELOPER, "EDR: gamma pipeline configured for linear output (exposure=%.2f, toneMapping=%d)\n", 
		           frag_spec_data.overbright, frag_spec_data.toneMapping );
	} else {
		...
		frag_spec_data.toneMapping = 0; // No tone mapping for SDR
	}
	...
	spec_entries[11].offset = offsetof(struct FragSpecData, toneMapping);
	spec_entries[11].size = sizeof(frag_spec_data.toneMapping);
...
void vk_bind_descriptor_sets( void )
{
	...
	// Update texture switches counter
	tr.frameStats.textureSwitches++;
...
void vk_bind_pipeline( uint32_t pipeline ) {
	...
	if ( vkpipe != vk.cmd->last_pipeline ) {
		...
		// Update shader switches counter
		tr.frameStats.shaderSwitches++;
	}
...
void vk_draw_geometry( Vk_Depth_Range depth_range, qboolean indexed ) {
	...
	// Update frame profiling stats
	tr.frameStats.drawCalls++;
	if (indexed) {
		tr.frameStats.trianglesDrawn += vk.cmd->num_indexes / 3;
	} else {
		tr.frameStats.trianglesDrawn += tess.numVertexes / 3;
	}
...
void vk_begin_main_render_pass( void )
{
	...
	// Use framebuffer dimensions (gls.windowWidth/windowHeight) for renderArea
	// to match the actual framebuffer size, especially important on macOS with HiDPI
	// where glConfig.vidWidth/vidHeight may differ from framebuffer size
	if ( r_fbo->integer == 0 )
	{
		// Framebuffer uses swapchain images which match gls.windowWidth/windowHeight
		vk.renderWidth = gls.windowWidth;
		vk.renderHeight = gls.windowHeight;
	}
	else
	{
		// Framebuffer uses FBO which matches glConfig.vidWidth/vidHeight
...
{
	// Safety check: don't use bloom resources if they weren't created
	if ( vk.render_pass.post_bloom == VK_NULL_HANDLE )
	{
		return;
	}
...
{
	// Safety check: don't use bloom resources if they weren't created
	if ( index >= ARRAY_LEN( vk.framebuffers.blur ) ||
		 vk.framebuffers.blur[index] == VK_NULL_HANDLE ||
		 index >= ARRAY_LEN( vk.render_pass.blur ) ||
		 vk.render_pass.blur[index] == VK_NULL_HANDLE )
	{
		return;
	}
...
void vk_end_frame( void )
{
	...
	if ( vk.fboActive )
	{
		...
		// Only use bloom if render passes were created (not on first init before UI loads)
		if ( r_bloom->integer && vk.render_pass.post_bloom != VK_NULL_HANDLE )
		{
			vk_bloom();
		}
...
qboolean vk_bloom( void )
{
	...
	// Early exit if bloom resources were not created (e.g., on first init before UI loads)
	if ( vk.render_pass.bloom_extract == VK_NULL_HANDLE ||
		 vk.render_pass.post_bloom == VK_NULL_HANDLE ||
		 vk.framebuffers.bloom_extract == VK_NULL_HANDLE ||
		 vk.bloom_extract_pipeline == VK_NULL_HANDLE )
	...
	for ( i = 0; i < VK_NUM_BLOOM_PASSES*2; i+=2 ) {
		// Safety check: ensure blur pipeline and descriptor exist
		if ( i+0 >= ARRAY_LEN( vk.blur_pipeline ) || vk.blur_pipeline[i+0] == VK_NULL_HANDLE ||
			 i+0 >= ARRAY_LEN( vk.bloom_image_descriptor ) || vk.bloom_image_descriptor[i+0] == VK_NULL_HANDLE )
		{
			break;
		}
		...
		// Safety check: ensure blur pipeline and descriptor exist for vertical blur
		if ( i+1 >= ARRAY_LEN( vk.blur_pipeline ) || vk.blur_pipeline[i+1] == VK_NULL_HANDLE ||
			 i+1 >= ARRAY_LEN( vk.bloom_image_descriptor ) || vk.bloom_image_descriptor[i+1] == VK_NULL_HANDLE )
		{
			break;
		}
	...
	// Safety check: ensure post-bloom resources exist
	if ( vk.render_pass.post_bloom != VK_NULL_HANDLE && vk.bloom_blend_pipeline != VK_NULL_HANDLE )
	{
		VkDescriptorSet dset[VK_NUM_BLOOM_PASSES];
		qboolean all_descriptors_valid = qtrue;

		for ( i = 0; i < VK_NUM_BLOOM_PASSES; i++ )
		{
			uint32_t desc_idx = (i+1)*2;
			if ( desc_idx >= ARRAY_LEN( vk.bloom_image_descriptor ) || vk.bloom_image_descriptor[desc_idx] == VK_NULL_HANDLE )
			{
				all_descriptors_valid = qfalse;
				break;
			}
			dset[i] = vk.bloom_image_descriptor[desc_idx];
		}
