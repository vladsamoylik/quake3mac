...
void RE_BeginFrame( stereoFrame_t stereoFrame ) {
	...
	// Initialize frame profiling
	tr.frameStats.startTime = Sys_Milliseconds();
	tr.frameStats.trianglesDrawn = 0;
	tr.frameStats.drawCalls = 0;
	tr.frameStats.textureSwitches = 0;
	tr.frameStats.shaderSwitches = 0;
	tr.frameStats.vertexBufferUploads = 0;
	tr.frameStats.indexBufferUploads = 0;
...
void RE_EndFrame( int *frontEndMsec, int *backEndMsec ) {
	...
	// Finalize frame profiling
	tr.frameStats.endTime = Sys_Milliseconds();
	int frameTime = tr.frameStats.endTime - tr.frameStats.startTime;

	// Log slow frames for performance analysis
	if (frameTime > 16) { // > 60 FPS
		ri.Printf(PRINT_DEVELOPER, "Slow frame: %dms (tris: %d, draws: %d, tex_sw: %d, sh_sw: %d)\n",
				 frameTime, tr.frameStats.trianglesDrawn, tr.frameStats.drawCalls,
				 tr.frameStats.textureSwitches, tr.frameStats.shaderSwitches);
	}

	// Clean up LRU texture cache periodically - DISABLED due to texture corruption issues
	// if ( (tr.frameCount & 1023) == 0 ) {  // every 1024 frames
