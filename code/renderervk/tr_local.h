...
#define MAX_TEXTURE_SIZE	8192 // must be less or equal to 32768 - increased for upscaled textures
...
typedef struct image_s {
	...
	int			lastUsedTime;		// for LRU cache management
...
typedef struct {
	...
	struct {
		...
		int					textureSwitches;
		int					shaderSwitches;
		int					vertexBufferUploads;
		int					indexBufferUploads;
		
		// Extended profiling metrics
		int					cpuTimeTotal;		// Total CPU time in microseconds
		int					cpuTimeRenderer;	// Renderer CPU time
		int					cpuTimeGame;		// Game logic time
		int					cpuTimeNetwork;		// Network processing time
		int					cpuTimeSound;		// Sound processing time
		
		int					memoryUsedTotal;	// Total memory used in bytes
		int					memoryUsedTextures;	// Texture memory
		int					memoryUsedGeometry;	// Geometry buffers
		int					memoryUsedSound;	// Sound buffers
		
		int					networkBytesIn;		// Bytes received this frame
		int					networkBytesOut;	// Bytes sent this frame
		int					networkPacketsIn;	// Packets received
		int					networkPacketsOut;	// Packets sent
		int					networkLatency;		// Current latency in ms
		
		int					fileReads;			// File I/O operations
		int					fileBytesRead;		// Bytes read from files
		int					fileCacheHits;		// File cache hits
		int					fileCacheMisses;	// File cache misses
	} frameStats;
} trGlobals_t;
...
extern cvar_t	*r_edr;
extern cvar_t	*r_edr_toneMapping;
...
void	VK_SelectTexture( int unit );
void	VK_TextureMode( const char *string );
void	VK_CheckErrors( void );
void	VK_State( unsigned stateVector );
void	VK_ClientState( int unit, unsigned stateVector );
